...
services=(
    "telnet:telnet-server:23:Telnet не зашифрованный протокол передачи данных. Рекомендуется использовать SSH."
...
for service_str in "${services[@]}"; do
    ...
    if rpm -qa | grep -q "${service[1]}"; then
...
for version in "${ssl_versions[@]}"; do
    if [ -d "/etc/httpd" ] && find /etc/httpd -type f | xargs grep -qw "$version"; then
